...
interface PopupProps {
  ...
  dataValue?: {
    agenda?: { items: string[] };
    glossary?: string;
    meetingDetails?: { location: string; participants: string };
    minutesDetails?: {};
  };
  stateValue?: boolean;
  toast?: any;
...
const BasicOneButtonAlert: React.FC<PopupProps> = ({
  ...
  const getHeaderColor = () => {
    const headerColor = messageHeaderColor
      ? colorMapping[messageHeaderColor]
      : "#FFCE85";
  ...
  const handleClick = async () => {
    ...
    if (dataValue.hasOwnProperty("glossary")) {
      ...
      var response = await buttonFunction(
        minutesID,
        chatHistoryID,
        dataValue.meetingDetails.location,
        dataValue.meetingDetails.participants
      );
      if (response !== undefined) {
        console.log(response.ERROR);
        setTimeout(() => {
      ...
      //@ts-ignore
      var response = await buttonFunction(
        dataValue.minutesDetails.reqData, //@ts-ignore
        toast, //@ts-ignore
