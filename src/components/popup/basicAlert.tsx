...
const getIcon = () => {
  if (alertIcon && Icons[alertIcon]) {
    const IconComponent = Icons[alertIcon];
    return <IconComponent alt={alertIcon} color={iconColor} size="small" />;
  }

  return <Icons.NotificationIcon alt="NotificationIcon" color="orange" size="small" />;
};
    ...
    <div className={`${styles.Notif} ${styles[colorVariant + 'Notif']}`}>
        <span className={styles.topRow}>
            <span className={styles.titleCol} >
                {getIcon()}
                <p className={styles.messageHeader} style={getHeaderColor()}>{messageTitle}</p>
            </span>
